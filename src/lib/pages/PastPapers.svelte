...
<article class="max-w-3xl">
  ...
  <div class="space-y-4">
    {#each questions as q (q.id)}
      <details class="card">
        ...
        <div class="mt-3 prose max-w-none">
          <h4>Question</h4>
          <pre class="whitespace-pre-wrap">{q.question}</pre>
          <h4>Model answer</h4>
          <pre class="whitespace-pre-wrap">{q.answer}</pre>
        </div>
    ...
    {#if questions.length === 0 && !editing}
      <div class="text-sm text-ink-500 italic">No questions yet. Add one above.</div>
    {/if}
  </div>
</article>
