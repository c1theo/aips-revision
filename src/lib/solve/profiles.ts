...
export const profiles: AlgorithmProfile[] = [
  ...
  {
    viz: 'NormalForm', label: '🎲 Normal-form game (payoff matrix, dominance, NE)', module: 'adversarial',
    blurb: 'Simultaneous-move 2-player game analysis.',
    intents: ['MODEL', 'SOLVE', 'ANALYZE', 'IDENTIFY'],
    triggers: {
      strong: ['normal-form game', 'normal form game', 'payoff matrix', 'dominant strategy', 'dominated strategy', 'nash equilibrium', 'pure strategy', 'mixed strategy', "prisoner's dilemma", 'prisoners dilemma', 'matching pennies', 'stag hunt', 'battle of the sexes', 'chicken game'],
      medium: ['utility matrix', 'best response', 'iterated elimination', 'zero-sum', 'maximin', 'minimax (zero-sum)', 'simultaneous move'],
      weak: ['game matrix', 'payoff', 'strategy', 'dominant'],
      anti: ['minimax tree', 'alpha-beta', 'csp', 'cnf'],
    },
    patterns: [
      { pattern: /\bnormal[-\s]?form\s+game\b/i, weight: 80, label: 'normal-form game' },
      { pattern: /\bpayoff\s+matrix\b/i, weight: 70, label: 'payoff matrix' },
      { pattern: /\bdominant\s+strategy\b/i, weight: 60, label: 'dominant strategy' },
      { pattern: /\bnash\s+equilibri/i, weight: 65, label: 'Nash equilibrium' },
      { pattern: /\bu[_]?[XYP1]\s*\(/i, weight: 35, label: 'utility function notation' },
      { pattern: /\bN\s*=\s*\{/i, weight: 25, label: 'player set notation' },
    ],
    companions: ['MinimaxTree'],
  },
  {
    viz: 'SearchGraph', label: '🗺 Search on arbitrary graph (BFS/DFS/UCS/Greedy/A*/Weighted A*/IDS)', module: 'search',
    blurb: 'Any state-space graph with heuristics — trace any search algorithm.',
    intents: ['TRACE', 'SOLVE', 'COMPARE'],
    triggers: {
      strong: ['weighted a*', 'weighted astar', 'iterative deepening search', 'ids with cycle', 'ancestor cycle prevention', 'tree search', 'graph search'],
      medium: ['state-space graph', 'state space search', 'find a path', 'heuristic value h', 'step cost', 'edge cost', 'straight-line distance', 'manhattan distance'],
      weak: ['graph', 'navigate', 'path'],
      anti: ['csp', 'cnf', 'minimax', 'horn'],
    },
    patterns: [
      { pattern: /\bweighted\s+a\*?\b/i, weight: 70, label: 'Weighted A*' },
      { pattern: /\bh\s*\(\s*[A-Za-z_]\w*\s*\)\s*=\s*\d+/i, weight: 50, label: 'heuristic value table' },
      { pattern: /\bstep\s+cost\b/i, weight: 35, label: 'step cost' },
      { pattern: /\biterative\s+deepening\b/i, weight: 60, label: 'iterative deepening' },
      { pattern: /\bcycle\s+prevention\b/i, weight: 50, label: 'cycle prevention' },
    ],
    companions: ['SearchGrid', 'SearchTree'],
  },
  {
