...
export const vizExplanations: Partial<Record<VizKey, VizExplanation>> = {
  ...
  AC3: {
    whatItShows: 'Any small CSP — variables with arbitrary domains, **unary** constraints (node consistency), and **binary** constraints with arbitrary predicates (=, ≠, <, >, ≤, ≥, abs(...) op n). Two phases: (1) NC drops values violating unary constraints; (2) AC-3 processes the arc queue. Per-arc **revision counts** track which arcs are revisited.',
    howToUse: 'Pick a preset or write a custom CSP. Toggle **Apply NC first**. Choose **FIFO** (automatic) or **Manual** (you click which arc to revise next — directly answers the "find an order where an arc must be revised more than once" exam question). Step through and watch the revision-count panel below.',
    whatToLookFor: [
      'NC phase: unary constraints like `x1 != 2` and `x3 > 1` collapse domains *before* AC-3 sees the binary structure.',
      'REVISE: when an arc is popped, values without support are removed; if the domain shrinks, **incoming** arcs (X_k, X_i) are re-enqueued — never the outgoing one you just processed.',
      'Each binary constraint contributes **two directed arcs** — REVISE only prunes the *first* argument of the arc.',
      'In Manual mode: choose an arc ordering that triggers cascading shrinks of one domain twice — the arc that "feeds into" it must be re-revised.',
      'Switch presets to compare problem types: `<` chains, equality-graph (Australia), and scheduling.',
    ],
    relatedConcepts: ['Node consistency', 'Arc consistency', 'O(c·d³) complexity', 'MAC algorithm'],
  },
  CSPLab: {
    whatItShows: 'The full CSP solving pipeline on one screen: **NC → optional AC-3 preprocessing → backtracking** with selectable propagation (None / FC / MAC) and branching style (d-way / 2-way). The search tree grows as you step.',
    howToUse: 'Pick a preset or write a CSP. Choose propagation level, branching style, variable ordering (top / MRV / degree / MRV+degree / custom), value ordering (increasing / decreasing / LCV / custom). Click any node in the search-tree panel to jump to that point and inspect the assignment + live domains.',
    whatToLookFor: [
      'Toggle **None vs FC vs MAC** with all other settings fixed — watch the tree shrink dramatically.',
      'Toggle **d-way vs 2-way** — same final answer but different exploration; 2-way produces a binary tree where every right branch is `X ≠ v` with strong propagation.',
      'Toggle **MRV vs top-to-bottom** on a hard preset — node count usually plummets with MRV.',
      'LCV picks the value that **rules out the fewest neighbours** — often finds a solution on the first leaf without backtracking.',
      'Pre-search AC-3 can sometimes solve a CSP outright (a tree-CSP with directional AC) or detect global infeasibility before backtracking even starts.',
    ],
    relatedConcepts: ['Forward checking', 'MAC', 'Heuristics (MRV/degree/LCV)', '2-way vs d-way branching'],
  },
  NQueens: {
